...
#define MAX_OPENED_FDS (MAX_CMDS * 3 + 1)
...
int init_processus(processus_t* proc) {
    ...
    proc->pid = 0;
    memset(proc->argv, 0, sizeof(proc->argv));
    memset(proc->envp, 0, sizeof(proc->envp));
    proc->path = NULL;
    proc->stdin_fd = 0;
    proc->stdout_fd = 1;
    proc->stderr_fd = 2;
...
int launch_processus(processus_t* proc) {
    ...
    if (pid < 0) {
        ...
        if (proc->stdin_fd != 0) dup2(proc->stdin_fd, 0);
        if (proc->stdout_fd != 1) dup2(proc->stdout_fd, 1);
        if (proc->stderr_fd != 2) dup2(proc->stderr_fd, 2);

        if (proc->cf && proc->cf->cmdl) {
            for (int i = 0; i < MAX_OPENED_FDS; i++) {
                int fd = proc->cf->cmdl->opened_descriptors[i];
                if (fd >= 0) {
                    if (fd != proc->stdin_fd && fd != proc->stdout_fd && fd != proc->stderr_fd) {
                        close(fd);
                    }
                }
            }
...
processus_t* add_processus(command_line_t* cmdl, control_flow_mode_t mode) {
    if (!cmdl || cmdl->num_commands >= MAX_CMDS) return NULL;

    processus_t* proc = &cmdl->commands[cmdl->num_commands];
    if (init_processus(proc) < 0) return NULL;

    proc->cf = &cmdl->flow[cmdl->num_commands];
    proc->cf->proc = proc;
    proc->cf->cmdl = cmdl;
    ...
    if (cmdl->num_commands > 0) {
        ...
        if (mode == UNCONDITIONAL) {
            prev_cf->unconditionnal_next = proc->cf;
        } else if (mode == ON_SUCCESS) {
            prev_cf->on_success_next = proc->cf;
        } else if (mode == ON_FAILURE) {
            prev_cf->on_failure_next = proc->cf;
        }
    }
...
int init_command_line(command_line_t* cmdl) {
    ...
    memset(cmdl->tokens, 0, sizeof(cmdl->tokens));
    ...
    for (int i = 0; i < MAX_OPENED_FDS; i++) cmdl->opened_descriptors[i] = -1;
...
int launch_command_line(command_line_t* cmdl) {
    ...
    for (unsigned int i = 0; i < cmdl->num_commands; i++) {
        processus_t* proc = &cmdl->commands[i];
        control_flow_t* cf = &cmdl->flow[i];
        int should_run = 0;

        if (i == 0) {
            should_run = 1;
        } else {
            processus_t* prev_proc = &cmdl->commands[i - 1];
            control_flow_t* prev_cf = &cmdl->flow[i - 1];
            
            int prev_success = (WIFEXITED(prev_proc->status) && WEXITSTATUS(prev_proc->status) == 0);

            if (prev_cf->unconditionnal_next == cf) {
                should_run = 1;
            } else if (prev_cf->on_success_next == cf) {
                should_run = prev_success;
            } else if (prev_cf->on_failure_next == cf) {
                should_run = !prev_success;
            }
        }
